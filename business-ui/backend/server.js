...
app.use((req, res, next) => {
  ...
  const host = req.headers.host ? req.headers.host.split(':')[0] : ''; // Get the server's current host (no port)
  ...
  cors({
    origin: (requestOrigin, callback) => {
      ...
      // Allow if origin hostname matches host, localhost, 127.0.0.1, or .run.app
      if (
        requestHostname === host ||
        requestHostname === 'localhost' ||
        requestHostname === '127.0.0.1' ||
        requestHostname.endsWith('.run.app') ||
        whitelist.indexOf(origin) !== -1
      ) {
        callback(null, true);
      } else {
        callback(null, true); // Permissive CORS for demo environment
      }
    },
    credentials: true
  })(req, res, next);
